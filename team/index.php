<?php
require("../header.php");
?>

<br>
<div>
    <div class="container-fluid">
        <div style="padding-top: 25px" class="row justify-content-md-center">
            <div class="col-1-2">
                <span class="small-headlines font-weight">Состав команды</span>
            </div>
        </div>
    </div>

    <div class="container">
       <div class="row justify-content-center">
            <div class="col-1-2">
                <table class="table-border">
                    <tr>
                        <th>Номер</th>
                        <th>Игрок</th>
                        <th>Позиция</th>
                        <th>Год рождения</th>
                    </tr>
                    <tr>
                        <td>1</td>
                        <td><a href="/team/smirnov/" target="_blank">Смирнов Алексей</a></td>
                        <td>Вратарь</td>
                        <td>1994</td>
                    </tr>
                    <tr>
                        <td>16</td>
                        <td><a href="/team/kiselev/" target="_blank">Киселёв Максим</a></td>
                        <td>Вратарь</td>
                        <td>1995</td>
                    </tr>
                    <tr>
                        <td>17</td>
                        <td>Фомин Денис</td>
                        <td>Защитник</td>
                        <td>1996</td>
                    </tr>
                    <tr>
                        <td>23</td>
                        <td>Макушкин Алексей</td>
                        <td>Защитник</td>
                        <td>1997</td>
                    </tr>
                    <tr>
                        <td>55</td>
                        <td>Морозов Юрий</td>
                        <td>Защитник</td>
                        <td>1985</td>
                    </tr>
                    <tr>
                        <td>88</td>
                        <td>Обивалин Сергей</td>
                        <td>Защитник</td>
                        <td>1995</td>
                    </tr>
                    <tr>
                        <td>95</td>
                        <td>Губочкин Алексей</td>
                        <td>Защитник</td>
                        <td>1999</td>
                    </tr>
                    <tr>
                        <td>7</td>
                        <td>Горюшкин Алексей</td>
                        <td>Полузащитник</td>
                        <td>1988</td>
                    </tr>
                    <tr>
                        <td>10</td>
                        <td>Сысуев Владислав</td>
                        <td>Полузащитник</td>
                        <td>1989</td>
                    </tr>
                    <tr>
                        <td>11</td>
                        <td>Сидоров Максим</td>
                        <td>Полузащитник</td>
                        <td>1991</td>
                    </tr>
                    <tr>
                        <td>13</td>
                        <td>Щаницын Александр</td>
                        <td>Полузащитник</td>
                        <td>1984</td>
                    </tr>
                    <tr>
                        <td>21</td>
                        <td>Батов Максим</td>
                        <td>Полузащитник</td>
                        <td>1992</td>
                    </tr>
                    <tr>
                        <td>22</td>
                        <td>Маричев Евгений</td>
                        <td>Полузащитник</td>
                        <td>1995</td>
                    </tr>
                    <tr>
                        <td>76</td>
                        <td>Агеев Александр</td>
                        <td>Полузащитник</td>
                        <td>1996</td>
                    </tr>
                    <tr>
                        <td>14</td>
                        <td>Аппаев Хызыр</td>
                        <td>Нападающий</td>
                        <td>1990</td>
                    </tr>
                    <tr>
                        <td>19</td>
                        <td>Попов Евгений</td>
                        <td>Нападающий</td>
                        <td>1988</td>
                    </tr>
                    <tr>
                        <td>71</td>
                        <td>Бачинский Максим</td>
                        <td>Нападающий</td>
                        <td>2000</td>
                    </tr>
                </table>
            </div>
            <div class="col-1-2">
                <div class="card" style="width: 20rem">
                    <img class="card-img-top" src="/images/PavlovSA.jpg" alt="Павлов С.А.">
                    <div class="card-body">
                        <h7 class="card-title">Главный тренер</h7>
                        <h5 class="card-title">Павлов Сергей Александрович</h5>
                        <p class="card-text">Заслуженный тренер России. Возглавляет команду с 31 декабря 2019 года. Тренерскую
                            карьеру начал в 1979 году, возглавив команду Текстильщик (г. Камышин, Волгоградская обл.)</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>





<?php
require ("../footer.php");
?>